addLayer("en", {
    ...
    energy: {
        perSecond() {
            ...
            if (hasChallenge('inf', 11)) base = base.pow(1.03)
            return base;
        },
        effect() {
    ...
    buyables: {
        11: {
            ...
            cost(x) {
                ...
                cost = softcap(cost, Decimal.pow(2, 1024), new Decimal(1).add(new Decimal(0.3).times(cost.add(10).log(10).div(308))))
                return cost
            },
            effect(x) {
                ...
                if (hasUpgrade('inf', 13)) power = power.pow(1.01)
                return power
            },
            display() {
    ...
    tabFormat: {
        "Energy": {
            content: [
                ...
                ['display-text', function () { return `Enhance energy is capped at 1.80e308 until an upgrade is bought in the next layer.` }, { 'font-size': '14.4px', 'color': 'silver' }],
                "buyables"
            ],
        },
        "Upgrades": {
    ...
    doReset(resettingLayer) {
        let keep = []
        if (hasAchievement('ac', 142)) keep.push('upgrades')
        if (layers[resettingLayer].row > this.row) layerDataReset("en", keep)
        if (hasAchievement('ac', 142)) player.en.points = new Decimal(1)
    },
    resetsNothing() {
